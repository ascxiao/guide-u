...
export function IncidentFormSheet() {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await createIncidentReport(formData)
      if (result.error) {
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      ...
      >
        <div className="flex items-center justify-between px-8 pt-6 pb-4 shrink-0">
          <div>
            <SheetTitle className="text-lg font-semibold text-foreground">New Incident Report</SheetTitle>
            <p className="text-xs text-muted-foreground mt-0.5">
              Fill in the details to create an incident report entry.
            </p>
          </div>
          <Button
            type="button"
        ...
        <form id="incident-form" onSubmit={handleSubmit} className="flex min-h-0 flex-1 overflow-hidden">
          <div className="flex w-1/2 flex-col gap-4 overflow-y-auto border-r px-8 py-6">
            <p className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground">Incident Details</p>

            <div className="space-y-1.5">
              <Label htmlFor="title" className="text-sm font-medium">
                Title <span className="text-muted-foreground font-normal">(required)</span>
              </Label>
              <Input id="title" name="title" placeholder="e.g. Bullying incident at cafeteria" className="h-9" required />
            </div>
          ...
          <div className="flex w-1/2 flex-col gap-6 overflow-y-auto px-8 py-6">
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                <div className="col-span-2 space-y-1.5">
                  ...
                  <Input id="location" name="location" placeholder="e.g. Main Building - 2nd Floor" className="h-9" />
                </div>
                <div className="space-y-1.5">
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <p className="text-[11px] text-muted-foreground">
                  Files will be uploaded to the incident-report-images storage bucket.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center justify-end gap-2 px-8 py-4 shrink-0">
          ...
          <Button type="submit" form="incident-form" disabled={isPending} className="h-9 px-5 text-sm bg-[#006633] hover:bg-[#005229] text-white">
